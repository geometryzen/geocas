...
function isMultivector<T>(arg: unknown): arg is Multivector<T> {
    if (arg) {
        const duck = arg as Multivector<T>
        return typeof duck.extractGrade === 'function';
    }
    else {
        return false;
    }
...
function isMetric<T>(arg: Metric<T>): arg is Metric<T> {
...
function mv<T>(blades: Blade<T>[], algebra: Algebra<T>, metric: METRIC<T>, labels: string[]) {
    ...
    const that: Multivector<T> = {
        ...
        compress(fraction = 1e-12): Multivector<T> {
            const eps = field.mulByNumber(field.one, fraction);
            let max = field.zero;
